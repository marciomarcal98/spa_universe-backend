...
app.post("/register", (request, response) => {
    ...
    db.query(searchCommand, [user.email], (error, data) => {
        ...
        if(data.length !== 0) {
            response.json({ message: "Já existe um usuário cadastro com esse e-mail. Tente outro e-mail!", userExists: true })
            return
        }
        ...
        db.query(insertCommand, [user.name, user.email, user.password], (error) => {
            ...
            response.json({ message: "Usuário cadastro com sucesso!" })
        })
    })
})
